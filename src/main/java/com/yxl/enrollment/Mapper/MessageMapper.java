...
public interface MessageMapper extends Mapper<Message> {
    ...
    @Select("select * from message where to_email=#{email} order by message_time desc")
    List<Message> selectAllByEmail(String email);

    @Select("select * from message where to_email=#{email} and is_read=0 order by message_time desc")
